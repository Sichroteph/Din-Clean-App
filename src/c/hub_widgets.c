...
#define S_WIDGET_DEF_COUNT ((uint8_t)(sizeof(s_widget_defs)/sizeof(s_widget_defs[0])))
...
static void widget_update_proc(Layer *layer, GContext *ctx) {
  ...
  if (widget_id < S_WIDGET_DEF_COUNT && s_widget_defs[widget_id].draw) {
...
static void widget_select_handler(ClickRecognizerRef rec, void *context) {
  ...
  if (widget_id < S_WIDGET_DEF_COUNT && s_widget_defs[widget_id].page_count) {
